...
void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	info->name				= "Koliba Primary or Secondary";
	...
	info->num_params		= 3;
	info->explanation		= "Emphasize primary or secondary colors.";
}
...
void f0r_get_param_info(f0r_param_info_t* info, int param_index) {
	switch (param_index) {
		case 0:
			...
			info->explanation	= "Natural contrast of the primary (or secondary) farba.";
			break;
		case 1:
			info->name			= "Secondary";
			info->type			= F0R_PARAM_BOOL;
			info->explanation	= "Go secondary if checked, primary if not.";
			break;
		case 2:
			info->name			= "sRGB";
			info->type			= F0R_PARAM_BOOL;
...
void f0r_set_param_value(f0r_instance_t instance, f0r_param_t param, int param_index) {
	...
	if ((instance != NULL) && (param != NULL)) switch (param_index) {
		...
		case 1:
			...
			if (instance->alter			!= b) {
				instance->alter			 = b;
				instance->mallet.flags	 = (b) ? KOLIBA_SLUTSECONDARY : KOLIBA_SLUTPRIMARY;
				instance->changed		 = 1;
			}
			break;
		case 2:
