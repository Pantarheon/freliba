...
void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	info->name				= "Koliba All Natural Contrasts";
	...
	info->num_params		= 9;
	info->explanation		= "Control the natural contrast of all sLut vertices.";
}
...
void f0r_get_param_info(f0r_param_info_t* info, int param_index) {
	switch (param_index) {
		case 0:
			...
			info->explanation	= "Natural contrast of the black vertex.";
			break;
		case 1:
			info->name			= "White";
			info->type			= F0R_PARAM_DOUBLE;
			info->explanation	= "Natural contrast of the white vertex.";
			break;
		case 2:
			info->name			= "Red";
			info->type			= F0R_PARAM_DOUBLE;
...
void f0r_update(f0r_instance_t instance, double time, const KOLIBA_RGBA8PIXEL *inframe, KOLIBA_RGBA8PIXEL *outframe) {
	if ((instance != NULL) && (inframe != NULL) && (outframe != NULL)) {
		...
		if (instance->changed) {
			KOLIBA_ApplyNaturalContrasts(&instance->sLut, &instance->eLut);
