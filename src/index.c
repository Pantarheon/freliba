/*
	...
	the use of indexed FLUTs in Koliba. It needs to be
...
static const double base[] = {0.0, 1.0/3.0, 1.0 - (2.0/3.0)};
static const KOLIBA_FLBINDEX flbindex = {
	{0,0,0},
	{1,1,2},
	{1,1,2},
	{1,1,2},
...
void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	...
	info->num_params		= 1;
	info->explanation		= "Test the functionality of indexed FLUTs.";
}
...
void f0r_update(f0r_instance_t instance, double time, const KOLIBA_RGBA8PIXEL *inframe, KOLIBA_RGBA8PIXEL *outframe) {
	if ((instance != NULL) && (inframe != NULL) && (outframe != NULL)) {
		...
		if (instance->changed) {
			instance->flags		= KOLIBA_FlutFlags(KOLIBA_ConvertFlbindexToFlut(&instance->fLut, &flbindex, base));
			instance->changed	= 0;
		}
