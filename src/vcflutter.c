/*
	...
	the use of flutter in Koliba. It needs to be linked
...
typedef	struct _vcflutter_instance {
...
} vcflutter_instance, *f0r_instance_t;
...
void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	...
	info->num_params		= 5;
	info->explanation		= "Applies Flutter to Vampyrectomy and Chromatomorphosis.";
}
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
	...
	if ((instance = malloc(sizeof(vcflutter_instance))) != NULL) {
...
void f0r_update(f0r_instance_t instance, double time, const KOLIBA_RGBA8PIXEL *inframe, KOLIBA_RGBA8PIXEL *outframe) {
	if ((instance != NULL) && (inframe != NULL) && (outframe != NULL)) {
		...
		if (instance->changed) {
			...
			KOLIBA_FlutEfficacy(&fLut, (KOLIBA_FLUT *)&iLut, instance->efficacy);
			if (instance->swap)
				KOLIBA_Flutter(&instance->fLut, &instance->fLut, &fLut);
			else
				KOLIBA_Flutter(&instance->fLut, &fLut, &instance->fLut);
