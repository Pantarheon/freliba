...
} warmcold_instance, *f0r_instance_t;

typedef	void	*f0r_param_t;

void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	info->name				= "Koliba Warm or Cold";
	...
	info->num_params		= 3;
	info->explanation		= "Make colors warmer or colder.";
}
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
	...
	if ((instance = malloc(sizeof(warmcold_instance))) != NULL) {
		KOLIBA_InitializeMallet(&instance->mallet, KOLIBA_SLUTWARM);
...
void f0r_get_param_info(f0r_param_info_t* info, int param_index) {
	switch (param_index) {
		case 0:
			...
			info->explanation	= "Natural contrast of the warm (or cold) farba.";
			break;
		case 1:
			info->name			= "Cold";
			info->type			= F0R_PARAM_BOOL;
			info->explanation	= "Go cold if checked, warm if not.";
			break;
		case 2:
			info->name			= "sRGB";
			info->type			= F0R_PARAM_BOOL;
...
void f0r_set_param_value(f0r_instance_t instance, f0r_param_t param, int param_index) {
	...
	if ((instance != NULL) && (param != NULL)) switch (param_index) {
		...
		case 1:
			...
			if (instance->cold			!= b) {
				instance->cold			 = b;
				instance->mallet.flags	 = (b) ? KOLIBA_SLUTCOLD : KOLIBA_SLUTWARM;
				instance->changed		 = 1;
			}
			break;
		case 2:
	...
	if ((instance != NULL) && (param != NULL)) switch(param_index) {
		case 0:
			*(double *)param = (double)instance->mallet.natcon;
			break;
		case 1:
			*(double *)param = (double)instance->cold;
			break;
		case 2:
