...
void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	info->name				= "Koliba Selene or Nyx";
	...
	info->num_params		= 3;
	info->explanation		= "Emphasize Selene or Nyx colors.";
}
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
	...
	if ((instance = malloc(sizeof(warmalter_instance))) != NULL) {
		KOLIBA_InitializeMallet(&instance->mallet, KOLIBA_SLUTSELENE);
...
void f0r_get_param_info(f0r_param_info_t* info, int param_index) {
	switch (param_index) {
		case 0:
			...
			info->explanation	= "Natural contrast of the Selene (or Nyx) farba.";
			break;
		case 1:
			info->name			= "Nyx";
			info->type			= F0R_PARAM_BOOL;
			info->explanation	= "Go Nyx if checked, Selene if not.";
			break;
		case 2:
			info->name			= "sRGB";
			info->type			= F0R_PARAM_BOOL;
...
void f0r_set_param_value(f0r_instance_t instance, f0r_param_t param, int param_index) {
	...
	if ((instance != NULL) && (param != NULL)) switch (param_index) {
		...
		case 1:
			...
			if (instance->alter			!= b) {
				instance->alter			 = b;
				instance->mallet.flags	 = (b) ? KOLIBA_SLUTNYX : KOLIBA_SLUTSELENE;
				instance->changed		 = 1;
			}
			break;
		case 2:
