...
void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	info->name				= "Koliba Bleach Bypass";
	...
	info->num_params		= 3;
	info->explanation		= "Bleach bypass.";
}
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
	...
	if ((instance = malloc(sizeof(erythropy_instance))) != NULL) {
		...
		KOLIBA_InitializeMallet(instance->mallet+1, KOLIBA_SLUTPRIMARY);

		instance->mallet[0].natcon		= 0.45;
		instance->mallet[1].saturation	= 0.5;
...
void f0r_get_param_info(f0r_param_info_t* info, int param_index) {
	switch (param_index) {
		case 0:
			...
			info->explanation	= "Natural contrast is specific to every svit and farba.";
			break;
		case 1:
			info->name			= "Saturation";
			info->type			= F0R_PARAM_DOUBLE;
			info->explanation	= "About one half is a good value here.";
			break;
		case 2:
			info->name			= "sRGB";
			info->type			= F0R_PARAM_BOOL;
			info->explanation	= "Accomodates the sRGB model.";
			break;
	}
}
...
void f0r_set_param_value(f0r_instance_t instance, f0r_param_t param, int param_index) {
	if ((instance != NULL) && (param != NULL)) switch (param_index) {
		case 0:
			if (instance->mallet[0].natcon		!= *(double *)param) {
				instance->mallet[0].natcon		 = *(double *)param;
				instance->changed				 = 1;
			}
			break;
		case 1:
			if (instance->mallet[1].saturation	!= *(double *)param) {
				instance->mallet[1].saturation	 = *(double *)param;
				instance->changed				 = 1;
			}
			break;
		case 2:
...
void f0r_get_param_value(f0r_instance_t instance, f0r_param_t param, int param_index) {
	if ((instance != NULL) && (param != NULL)) switch(param_index) {
		case 0:
			*(double *)param = (double)instance->mallet[0].natcon;
			break;
		case 1:
			*(double *)param = (double)instance->mallet[1].saturation;
			break;
		case 2:
