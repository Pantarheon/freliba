/*
	...
	the use of fLut chaining in Koliba. It needs to be linked
...
void f0r_get_plugin_info(f0r_plugin_info_t* info) {
	...
	info->num_params		= 5;
	info->explanation		= "Chains Vampyrectomy and Chromatomorphosis.";
}
...
void f0r_get_param_info(f0r_param_info_t* info, int param_index) {
	switch (param_index) {
		...
		case 3:
			...
			info->explanation	= "Swaps the order of the two effects.";
			break;
		case 4:
			info->name			= "sRGB";
			info->type			= F0R_PARAM_BOOL;
	...
	if ((instance != NULL) && (param != NULL)) switch(param_index) {
		...
		case 2:
			*(double *)param = (double)instance->con[1];
			break;
		case 3:
			*(double *)param = (double)instance->swap;
			break;
		case 4:
...
void f0r_update(f0r_instance_t instance, double time, const KOLIBA_RGBA8PIXEL *inframe, KOLIBA_RGBA8PIXEL *outframe) {
	if ((instance != NULL) && (inframe != NULL) && (outframe != NULL)) {
		...
		if (instance->changed) {
			...
			KOLIBA_ConvertSlutToFlut(&(instance->fLut[0]), &instance->vert);
			KOLIBA_FlutEfficacy(&(instance->fLut[1]), (KOLIBA_FLUT *)&iLut, instance->efficacy);
			if (instance->swap) {
				instance->fChain[0].fLut = &(instance->fLut[0]);
				instance->fChain[1].fLut = &(instance->fLut[1]);
			}
			else {
				instance->fChain[0].fLut = &(instance->fLut[1]);
				instance->fChain[1].fLut = &instance->fLut[0];
			}
			instance->fChain[0].flags	= KOLIBA_FlutFlags(instance->fChain[0].fLut);
		...
		for (i = instance->count; i; i--, inframe++, outframe++) {
			KOLIBA_PolyRgba8Pixel(outframe, inframe, instance->fChain, 2, iconv, oconv)->a = inframe->a;
		}
	}
}
